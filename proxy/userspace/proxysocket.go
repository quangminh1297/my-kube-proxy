...
func newProxySocket(protocol v1.Protocol, ip net.IP, port int) (ProxySocket, error) {
	...
	if ip != nil {
		host = ip.String()
		klog.V(0).Infof("=========NUMBER OF HOST==========", host)
	}
...
func (tcp *tcpProxySocket) ListenPort() int {
	//
	klog.V(0).Infof("==========PRINT-ListenPORT=============", tcp.port) // My-config
	//
	return tcp.port
}
//map[nodename][]ep
...
//		endpoint, err := loadBalancer.NextEndpoint(nodename, service, srcAddr, sessionAffinityReset)
		...
		//
		//endpoint = "10.244.1.18:80" // my-config
		//endpoint = [10.244.1.54:8080 10.244.1.55:8080 10.244.2.33:8080] // my-config
		klog.V(0).Infof("=========STRING FORMAT======", endpoint)
		//
...
func (tcp *tcpProxySocket) ProxyLoop(service proxy.ServicePortName, myInfo *ServiceInfo, loadBalancer LoadBalancer) {
	for {
		...
		klog.V(0).Infof("---|| ADD-ProxyLoop || inConn.LocalAddr().String():-->",inConn.LocalAddr().String())
		klog.V(0).Infof("---|| ADD-ProxyLoop || inConn.RemoteAddr().String():-->",inConn.RemoteAddr().String())
		...
		klog.V(0).Infof("---|| ADD-ProxyLoop || outConn.LocalAddr().String():-->",outConn.LocalAddr().String())
		klog.V(0).Infof("---|| ADD-ProxyLoop || outConn.RemoteAddr().String():-->",outConn.RemoteAddr().String())
//		if conn == node1 {
//			outConn => 1 trong cac ep trong node1
//		}
		if err != nil {
